...
class VertexCoverPenaltyCircuitBuilder:
    ...
    def build(self) -> QuantumCircuit:
        ...
        for _ in range(self.config.grover_iterations):
            ...
            # 1. Compute state in counter register
            self._weight_sum(circuit, search_space_reg, binary_counter_reg)
            ...
            self._apply_x_to_vertices_register(circuit, search_space_reg)
            self._weight_sum(circuit, search_space_reg, binary_counter_reg, inverse=True)
            self._barrier(circuit)
    ...
    def _weight_sum(self, circuit, bits_reg, count_reg, inverse=False):
        iter_bits = range(bits_reg.size) if not inverse else range(bits_reg.size - 1, -1, -1)
        
        for k in iter_bits:
            weight = self.config.vertex_weights[k]
            # Decompose weight into bits
            weight_bits = []
            temp_val = weight
            bit_idx = 0
            while temp_val > 0:
            ...
            p_bits_iter = weight_bits if not inverse else reversed(weight_bits)
            for p_bit in p_bits_iter:
                if p_bit < count_reg.size:
                    target_qubits = [count_reg[i] for i in range(p_bit, count_reg.size)]
                    self._increment(circuit, target_qubits, control_qubits=[bits_reg[k]], inverse=inverse)
                    self._barrier(circuit)

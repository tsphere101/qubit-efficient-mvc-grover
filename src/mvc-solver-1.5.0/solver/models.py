...
class SolverConfig:
    ...
    def __post_init__(self):
        ...
        if len(self.vertex_weights) < self.num_vertices:
             # pad with ones if not provided for all
             self.vertex_weights.extend([1] * (self.num_vertices - len(self.vertex_weights)))

        # New Weighted Penalty: P = sum(w_i) + 1
        self.penalty_amount: int = sum(self.vertex_weights) + 1
        
        # Max value in counter: sum(w_i) + num_edges * P
        self.counter_size: int = (sum(self.vertex_weights) + self.penalty_amount * self.num_edges).bit_length()
